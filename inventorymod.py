...
def list_low_stock():
    #print("[28;0H[94m📋 Showing inventory...[0m")
    low_items = []
    for product_id, details in products.items():
        if details["in_stock"] <= details["threshold"]:
            missing = details["threshold"] - details["in_stock"]
            low_items.append((details["name"], missing))

    if low_items:
        print("[95m[29;0H📦 Low stock detected:[0m")  # Move to line 29
        for idx, (item, missing) in enumerate(low_items):
            print(f"[{30 + idx};0H[96m- {item:<40}[91m Missing: {missing:>3}[0m")
    else:
        print("\033[29;0H✅ All stock levels are sufficient.")
