...
def open_live_camera():
    global close_live_camera
    close_live_camera = False
    ...
    while True:
        ...
        cv2.imshow("Capohm Live Camera - Press Q to Quit", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        if close_live_camera:
            break
    cap.release()
...
def camera_watch():
    ...
    crow_model = YOLO('home/terry/capohm/best.pt')  # Update with your actual path
    ...
    #crow_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_fullbody.xml')
    ...
    while True:
        ...
        results = crow_model(frame, verbose=False)
        for r in results:
            for box in r.boxes:
                cls_id = int(box.cls[0])
                if crow_model.names[cls_id].lower() == 'crow':
                    sys.stdout.write("\033[28;0H🪶 Crow detected!\n")
                    speak("Warning. Crow detected in the area.")
                    # Optional: break to avoid repeats per frame
        ...
        #crows = crow_cascade.detectMultiScale(gray, 1.1, 3)

        #if len(crows) > 0:
        #    assistant_output("🪶 Crow detected! Stay sharp.")
        #    speak("Warning. Crow detected in the area.")
        #    time.sleep(5)  # Avoid spamming the alert
...
def chat():
    ...
    threading.Thread(target=open_live_camera, daemon=True).start()
    ...
    while True:
        ...
        if user_input.lower() in ["open camera", "show camera"]:
            speak("Opening live camera feed. Say 'close camera' to stop.")
            open_live_camera()
            continue

        if user_input.lower() in ["close camera", "stop camera"]:
            speak("Closing camera view.")
            close_live_camera = True
            continue
